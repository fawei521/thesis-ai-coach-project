...
def t_power():
    print("\n【8/29】开题样本量 / 功效估算（G*Power 等价，回答要发多少份）")
...
def t_refs():
    print("\n【16/29】参考文献格式化（题录CSV → GB/T 7714-2015 编号列表，可直接粘进论文）")
...
def t_ppt():
    print("\n【21/29】把大纲排成 PPT（开题/答辩汇报 .pptx，只排版不代写）")
...
def t_workspace():
    print("\n【22/29】检查并补齐「我的工作区」：各类目录 ＋ 你的进度卡与检索记录")
    print("  只新增缺的目录、缺的文件，绝不重命名/移动/删除/覆盖你已有的东西；重复跑没有副作用。")
    print("  进度卡与检索记录这两份要你填的文件**不在发布包里**（装了会盖掉你的记录），")
    print("  这一步会把包内空白模板复制成你的那一份；已经填过的，一个字都不动。")
