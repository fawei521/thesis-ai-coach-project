...
def t_power():
    print("\n【8/17】开题样本量 / 功效估算（G*Power 等价，回答要发多少份）")
...
def t_anonymize():
    print("\n【11/17】数据去标识化（发给AI/上传/给外校前，隐去姓名学号手机等）")
...
def t_effect():
    print("\n【12/17】效应量换算与复核（写结果时，由 t/F/χ²/r 或均值标准差算效应量）")
...
def t_itemanalysis():
    print("\n【14/17】预试问卷项目分析（决断值CR + CITC + 删题后α，决定删改哪些题）")
...
def t_cvi():
    print("\n【15/17】自编量表内容效度 CVI（专家相关性评分 → I-CVI/κ*/S-CVI）")
...
def t_refs():
    print("\n【16/17】参考文献格式化（题录CSV → GB/T 7714-2015 编号列表，可直接粘进论文）")
...
def t_missing():
    print("\n【17/17】缺失值分析与 Little's MCAR 检验（缺多少/怎么缺/能不能直接删）")
