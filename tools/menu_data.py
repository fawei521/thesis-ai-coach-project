...
def t_anonymize():
    print("\n【11/29】数据去标识化（发给AI/上传/给外校前，隐去姓名学号手机等）")
...
def t_effect():
    print("\n【12/29】效应量换算与复核（写结果时，由 t/F/χ²/r 或均值标准差算效应量）")
...
def t_itemanalysis():
    print("\n【14/29】预试问卷项目分析（决断值CR + CITC + 删题后α，决定删改哪些题）")
...
def t_cvi():
    print("\n【15/29】自编量表内容效度 CVI（专家相关性评分 → I-CVI/κ*/S-CVI）")
...
def t_missing():
    print("\n【17/29】缺失值分析与 Little's MCAR 检验（缺多少/怎么缺/能不能直接删）")
...
def t_paired():
    print("\n【19/29】配对设计差异检验（前后测 / 两条件：配对t＋d_z＋差值正态性＋Wilcoxon）")
...
def t_multcomp():
    print("\n【20/29】多重比较校正（Bonferroni / Holm / BH-FDR / BY-FDR）")
