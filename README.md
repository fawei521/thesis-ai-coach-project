...
> ② **豆包 Skill 轻量版**（`doubao-skill/` 目录，Skill v1.7）——可安装到豆包技能目录，只保留分阶段引导协议、身份陪伴边界、鼓励系统与参考资料，**不含脚本**，并对手机做了原生适配，适合不想下载整个项目包、或主要在手机上推进的同学；需要自动化时轻量版会指引回到完整版。两版学术口径一致。
...
**当前版本：v1.93**（2026-09-20）**开发文件不再进发布包**：`tests/` 与开发流程文档从包里去掉（`.gitattributes` 的 `export-ignore`，仓库里一行不少）。理由不是体积——整包才 1 MB——而是**学生的 AI 会不会读到**：入口文档里的维护者模式块会教它去跑三五分钟的回归，而严谨性实验的夹具里放着**为考 AI 而故意编的假信度系数**，长得和真数据一样，通读文件夹就可能被引进开题（这正是本版要防的那类错）。包从 184 件 / 2495 KB 降到 **139 件 / 2070 KB**；包形态从此有断言盯（必带的 26 件一件不能少、假数字一个字节不外发），发版验收相应改成"学生包 ＋ 只补回 `tests/` 的验证副本"。想看源码的人请直接取仓库。

**上一个版本：v1.92**（2026-09-20）**装新包不再盖掉你的进度卡与检索记录**：以前发布包里带着 `我的工作区/我的论文进度.md` 与 `我的工作区/01-文献PDF/检索记录.md` 两份空白件，而学生填的就是同一个路径——**装新版等于把自己的存档点盖回空白**，且使用副本没有 `.git`、盖了不可回滚（2026-09-20 实测）。现在包里只留 `templates/` 里的空白基线，那两份改由菜单第 22 项**缺才生成、已存在一个字都不动**，并排除在仓库之外（顺带堵住“姓名/导师可能被提交进包”这个隐私口子）。`QUICKSTART.md` 新增第八节「拿到新版怎么装」：解压到一个新空文件夹 → 把旧 `我的工作区` 整个**剪切**过去 → 跑第 22 项检查；不要再直接覆盖解压。另修正 README 里停在旧号的豆包 Skill 版本串，并加断言盯它不再漂。
