...
**当前版本：v1.60**（2026-09-18）测量学闭环补齐——自编/修订量表从内容效度到结构效度的工具缺口补齐，菜单增至 15 项
- **聚合/区分效度**：新增 `validity_cr_ave.py`（菜单第 13 项），由 CFA 标准化载荷算组合信度 CR、平均方差抽取 AVE、√AVE 并做 Fornell-Larcker 区分效度判定（单因子下 CR 与 McDonald's ω 等价）；载荷须来自真实 CFA
- **预试项目分析**：新增 `item_analysis.py`（菜单第 14 项），高低 27% 独立样本 t 决断值 CR＋CITC＋删题后 α＋保留/讨论删改判定，只提示不替学生删题
- **内容效度 CVI**：新增 `content_cvi.py`（菜单第 15 项），专家 1–4 相关性评分→逐条 I-CVI、机遇校正 κ*、量表 S-CVI/Ave 与 S-CVI/UA，按 Lynn/Polit-Beck 阈值给建议（仅自编/修订量表需要）
- **样本量补 t 设计**：`sample_size.py`（菜单第 8 项）借 t²=F 精确等价新增独立两样本 t 与配对/单样本 t，最小 N 经 scipy 非中心 t 逐位核对（独立 d=.5 总 128/每组 64；配对 dz=.5 N=34）
- **量表库扩到 29 组（10 大类）**：v1.60 新增 PANAS 正负情绪、IRI-C 共情、GQ-6 感恩、GHQ-12 一般健康、PSQI 睡眠质量（新开“睡眠与心身健康类”）；题数/计分/分级硬事实逐条联网核查，口径有出入处标注“以所引原文为准”
- **共同方法偏差方法学加固**：明确 Harman 仅最宽松事后检验，补程序控制（Podsakoff 等 2003）与 ULMC、标记变量法（Lindell & Whitney 2001）路径与工具边界

**上一个版本：v1.59** 数据隐私与统计严谨性：去标识化隐私闸 `anonymize_data.py`（菜单 11，k-匿名体检、绝不覆盖原始数据）＋多元异常值 Mahalanobis `--mahalanobis` 只标记不删除＋效应量换算复核 `effect_size.py`（菜单 12）＋量表库扩至 24 组 9 大类＋问答式「统计方法选择器」网页范例。
