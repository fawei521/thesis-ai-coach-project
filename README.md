...
├── core/                     # AI规则 + AI素养
├── workflows/                # 9个阶段工作流手册
├── tools/                    # 9个脚本（含统一菜单menu.py）+ stats/ 统计实现包（9个模块）
├── psychology/               # 量表/统计/伦理知识库
├── templates/                # 问卷/大纲/开题/答辩/进度卡/AI声明模板
└── tests/                    # full_e2e.py 一键全量回归、consistency_check.py 文档↔代码一致性自检、专项测试与测试数据
```

> 维护者/接手者：改动后运行 `python tests/full_e2e.py`（约3-5分钟，138项起，自动备份恢复测试数据），退出码 0 才算通过；学生日常使用不需要跑。

## 版本

**当前版本：v1.53.1**（2026-09-17）
...
**v1.53.1 工程化版（当前）**
- **修复中文 Windows 管道运行崩溃**：9 个工具脚本与 4 个测试脚本统一加"输出编码守卫"，
  管道/重定向时强制 UTF-8。此前 `² χ² ⚠ ↔` 等 GBK 无法编码的字符会让
  `auto_stats.py`、`consistency_check.py`、`full_e2e.py` 直接抛 `UnicodeEncodeError` 退出——
  而这正是"AI 助手跑脚本读输出"的必经路径（交互式控制台不受影响）
- **`tools/auto_stats.py` 拆包**：单文件 2694 行 → `tools/stats/` 包 9 个模块 + 瘦身 CLI。
  分工：`mathx`（统计量与分布函数）/ `linalg` / `dataio` / `desc` / `reliability` /
  `plots` / `efa` / `compare` / `regress`。函数体逐行未改动，**所有 CLI 开关与导出文件名保持不变**
- `consistency_check.py` 改为递归扫描 `tools/**/*.py`，拆包后仍能核对导出文件名与开关
- `full_e2e.py` 增至 138 项断言（新增编码守卫、管道不崩、拆包结构、runpy 可调用、CHANGELOG 齐备）
- 版本历史合并为单文件 `CHANGELOG.md`

**上一个版本：v1.53** 问卷模板人口学适配中学生/混合样本 —— 详见 [CHANGELOG.md](CHANGELOG.md)。
