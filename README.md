...
> ② **豆包 Skill 轻量版**（`doubao-skill/` 目录，Skill v1.8）——可安装到豆包技能目录，只保留分阶段引导协议、身份陪伴边界、鼓励系统与参考资料，**不含脚本**，并对手机做了原生适配，适合不想下载整个项目包、或主要在手机上推进的同学；需要自动化时轻量版会指引回到完整版。两版学术口径一致。
...
**当前版本：v1.94**（2026-09-21）**你要"你来出这份成果"，AI 就得交出全套**：以前"帮我做个开题 PPT"可能真就只给一个 PPT。现在按 `core/outcome-delivery.md`——先问截止日期与学校格式要求 → 把成果拆成一整套（开题＝报告＋PPT＋模型图＋讲稿＋问答＋自查）→ 清单交你确认 → 每一件先去找现成的专业技能（本包自带的 `outline_to_ppt.py` 明确降级为保底）→ 交付用 ✅/⚠️/❌ 三态回执，没做的必须写出来；紧急模式压缩的是流程与篇幅、**不是交付物的件数**。开题 PPT 的要求从"8 项清单"改成"内容底线＋视觉底线＋风格化"，模板新增一份"风格化作业单"（三样不定别开工，占位没换第 23 项会报缺项）。另新增**毕业材料总清单**：任务书、计划书、中期检查表、指导记录、外文翻译、承诺书、答辩记录、成绩评定表——学校要交的不止一篇论文，逐校不同，问过教务再逐项打勾；菜单第 22 项现在会生成第三份要你填的文件。外部技能只装项目级 `.qoder/skills/_external/` 并登记，装前过许可证／行为／你同意三道闸（**有的技能禁止复制分发，一个字节都不进包**）。

**上一个版本：v1.93**（2026-09-20）**开发文件不再进发布包**：`tests/` 与开发流程文档从包里去掉（`.gitattributes` 的 `export-ignore`，仓库里一行不少）。理由不是体积——整包才 1 MB——而是**学生的 AI 会不会读到**：入口文档里的维护者模式块会教它去跑三五分钟的回归，而严谨性实验的夹具里放着**为考 AI 而故意编的假信度系数**，长得和真数据一样，通读文件夹就可能被引进开题（这正是本版要防的那类错）。包从 184 件 / 2495 KB 降到 **139 件 / 2070 KB**；包形态从此有断言盯（必带的 26 件一件不能少、假数字一个字节不外发），发版验收相应改成"学生包 ＋ 只补回 `tests/` 的验证副本"。想看源码的人请直接取仓库。
