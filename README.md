...
**当前版本：v1.89**（2026-09-19）**轻量版口径追平（Skill v1.5）**：手机上那套引导补上开题就绪度自检、McDonald's ω、CR/AVE/HTMT、前提假设、多重比较校正、配对效应量、缺失机制（MCAR/MAR→MI/FIML）与回归诊断口径，并修掉"答辩 24 类高频问题"这个误写（完整版是 24 问分 7 类）；新增 `case_18.py` 盯住"轻量版点名的完整版脚本与菜单编号必须真实存在"。另拆掉一处哑雷：`full_e2e.py` 里一个裸 `\r` 把两条片段注册挤在同一显示行，任何文本工具都看不见它——现全仓行尾单一并有断言。**完整版学生侧命令与工具行为零变化**

**上一个版本：v1.88**（2026-09-19）修掉**手机侧一条发不出去的承诺**：轻量版 README 说合并单文件"保底一定能用"，但它被 `.gitignore` 当构建产物排除、而发布包由 `git archive` 生成 → 两个发布 zip 里都没有它；现改为入库随包分发，并把 `validate.py` 的"缺件照报通过"改成缺件判红。新增跨包守卫 `tests/skill_sync_check.py`：两侧硬口径逐项比对 + 版本自述只留一处（过去四处各写一遍已漂成三个数字）
