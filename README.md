...
**当前版本：v1.92**（2026-09-20）**装新包不再盖掉你的进度卡与检索记录**：以前发布包里带着 `我的工作区/我的论文进度.md` 与 `01-文献PDF/检索记录.md` 两份空白件，而学生填的就是同一个路径——**装新版等于把自己的存档点盖回空白**，且使用副本没有 `.git`、盖了不可回滚（2026-09-20 实测）。现在包里只留 `templates/` 里的空白基线，那两份改由菜单第 22 项**缺才生成、已存在一个字都不动**，并排除在仓库之外（顺带堵住“姓名/导师可能被提交进包”这个隐私口子）。`QUICKSTART.md` 新增第八节「拿到新版怎么装」：解压到一个新空文件夹 → 把旧 `我的工作区` 整个**剪切**过去 → 跑第 22 项检查；不要再直接覆盖解压。另修正 README 里停在旧号的豆包 Skill 版本串，并加断言盯它不再漂。
