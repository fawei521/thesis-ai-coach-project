...
**当前版本：v1.86**（2026-09-19）修掉一处**维护者侧的假失败**：输出编码守卫原先带 `and not sys.stdout.isatty()` 条件，而 `nul`/`/dev/null` 是字符设备、`isatty()` 在那里为真 → 守卫被跳过、退回 GBK，打印 `↔`/`χ²` 就崩，看起来像"检查器报错却没有任何内容"。现改为**无条件生效**（真控制台实测编码本就是 UTF-8，此改动对双击 bat 的学生零影响），并钉了 4 条断言；顺带对齐 `menu.py` 的分册注释。full_e2e 685→689 项

**上一个版本：v1.85** 新增**开题就绪度自检**（`proposal_readiness.py`／菜单第 23 项）：读你的开题大纲＋进度卡，按 `workflows/proposal-guide.md` 的口径分四档报**缺项/矛盾/风险/提示**，**只报问题、一个字都不替你写、不改任何文件**；full_e2e 676→685 项
