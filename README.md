...
**当前版本：v1.90**（2026-09-20）**证据与核验纪律：摘要不算原文**：新增权威源 `core/evidence-rigor.md`——三态标注（已核实/推断/未查到）、引用四要素（谁 / 逐字原文 / 第几节第几张表 / 读的是全文还是摘要）、**数字三查**（回读定位→对象对齐→换源复核）、"搜不到 ≠ 没有"、**更正的门槛高于原结论**、落盘的错必须清点。关键不在多写一遍"要严谨"，而是把它**接进每轮回复前门禁**（两份 coaching-protocol 的产出前清单各加三条硬动作）、IMRaD 卡片加"读取范围声明＋取证栏"、`proposal_readiness.py` 把没清的 `[需核实]` 报成缺项。轻量版同步出 **Skill v1.6**（镜像页＋T44–T46，validate 列为必备件）。**统计脚本的命令与数值零变化**

**上一个版本：v1.89**（2026-09-19）轻量版口径追平（Skill v1.5）：手机上那套引导补上开题就绪度自检、McDonald's ω、CR/AVE/HTMT、前提假设、多重比较校正、配对效应量、缺失机制（MCAR/MAR→MI/FIML）与回归诊断口径，并修掉"答辩 24 类高频问题"这个误写（完整版是 24 问分 7 类）；新增 `case_18.py` 盯住"轻量版点名的完整版脚本与菜单编号必须真实存在"，另拆掉 `full_e2e.py` 里一个裸 `\r` 把两条片段注册挤在同一显示行的哑雷
