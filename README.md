...
> ② **豆包 Skill 轻量版**（`doubao-skill/` 目录，Skill v1.9）——可安装到豆包技能目录，只保留分阶段引导协议、身份陪伴边界、鼓励系统与参考资料，**不含脚本**，并对手机做了原生适配，适合不想下载整个项目包、或主要在手机上推进的同学；需要自动化时轻量版会指引回到完整版。两版学术口径一致。
...
**当前版本：v1.98**（2026-09-22）**综述里的每条引用，先问一句"我有没有全文"**：新增菜单第 26 项 `lit_fetch.py`（拿清单逐篇探测公开全文，六态台账，落到 `01-文献PDF/原文/`；不绕付费墙、逐篇限速、单次上限 30 篇）与第 27 项 `lit_verify.py`（Crossref＋OpenAlex 两个独立登记处逐字段核题录，专抓编造标题/错刊名/猜错一位的 DOI；不生成文献、不替你补字段）。为什么现在做：`core/evidence-rigor.md` 把"读的是什么材料"列为引用四要素之一，**只有摘要时不得输出"该研究发现 X"级别的结论**——可此前"能不能拿到全文"只能靠人一篇篇点；一次真实作业里 35 篇有 18 篇其实有公开全文、12 篇只能走知网/图书馆，分不清就会把摘要当全文引。第 23 项同时把"模板没填的【】"与"核验标注的【】"分开数（以前混在一起，一份内容完整的报告会凭空多出十几处假缺项）。轻量版口径同步。
