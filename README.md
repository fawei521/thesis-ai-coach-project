...
**当前版本：v1.91**（2026-09-20）**把严谨性写进宪法，并做了一次真对照实验**：宪法第三条新增四条可判形制——**顺序不许反**（先贴逐字原文＋节号/表号再判断）、**空位可以留空**（原文没写就写"原文未报告"，不许拿别的研究的数填坑）、**交叉验证必须换独立来源**（另一个 AI 的复述、同一页第二次摘录、自己上一轮的记忆都不算第二源）、**流利不等于准确**；第十条加"**只把话写进文档＝未解决**"。新规矩"**上下文会被压缩，印象不是原文**"由 `tests/rigor_experiment.py` 的 A/B 实验逼出来：**实测三组（无规则／旧规则／新规则）都没报出错误的 α**，测得出的是形式可稽核性（三态标注新规则组 4/4、无规则组 2/3），而"学生催你只回一个数字"时装了规则也会掉形——缺陷已挂待办，没写成"已解决"。轻量版同步出 **Skill v1.7**（自测 T47/T48）。**统计脚本的命令与数值零变化**

**上一个版本：v1.90**（2026-09-20）**证据与核验纪律：摘要不算原文**：新增权威源 `core/evidence-rigor.md`（三态标注、引用四要素、切片禁令、**数字三查**、否定结论四步、更正门槛高于原结论、落盘的错必须清点），并把它**接进每轮回复前门禁**（两份 coaching-protocol 的产出前清单各加三条硬动作）；IMRaD 卡片加"读取范围声明＋取证栏"，`proposal_readiness.py` 把没清的 `[需核实]` 报成缺项；轻量版 Skill v1.6（镜像页＋T44–T46，`validate.py` 列为必备件）
