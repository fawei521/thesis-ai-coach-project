...
> 维护者/接手者：改动后运行 `python tests/full_e2e.py`（约3-5分钟，414 项断言，自动备份恢复测试数据），退出码 0 才算通过；学生日常使用不需要跑。

## 版本

**当前版本：v1.63**（2026-09-18）高频量表库再扩充（完整版；doubao-skill 本轮无改动）
- **量表库 29→39 组、10→11 大类**：`psychology/scale-library.md` 新增 10 组经联网核查的高频量表——坚毅 Grit-S、交往焦虑 IAS、人际信任 ITS、社会比较倾向/上行社会比较 INCOM、错失焦虑 FoMOs、自我同情 SCS-SF、积极心理资本 PPQ、经验性回避 AAQ-II、学习投入 UWES-S、学业倦怠（连榕量表/MBI-SS）；新开"十一、学习心理与教育情境类"，全文小节确定性重编号
- **硬事实纪律**：作者年份、题数、维度、反向题号、中文版出处逐条联网核查；题数/锚点/反向题存在版本分歧的（PPQ 5点/7点、FoMO 10题/8题版、UWES-S 0-6/1-7、连榕成就感低反向题）全部显式标注"以题本原文为准"，不凭记忆报数
- **测试与文档**：full_e2e 404→414 项（10 组新量表硬事实断言＋编号/大类断言更新），CHANGELOG、PROJECT_PLAN、START、e2e-test 同步

**上一个版本：v1.62** 现代信效度指标补齐：auto_stats 信度节自动报告 McDonald's ω（单因子 PAF，写入 `_信度分析.csv`）；validity_cr_ave.py 新增 HTMT 模式（原始问卷数据直算异质-单质比率，.85/.90 双门槛＋Bootstrap 95%CI 上限<1，导出 `_HTMT区分效度.csv`）；full_e2e 390→404 项；逐条见 CHANGELOG。
