...
> 维护者/接手者：改动后运行 `python tests/full_e2e.py`（约3-5分钟，163项起，自动备份恢复测试数据），退出码 0 才算通过；学生日常使用不需要跑。

## 版本

**当前版本：v1.54**（2026-09-17）
...
**上一个版本：v1.53.1** 管道编码崩溃修复 + auto_stats 拆包 + 版本日志单文件化 —— 详见 [CHANGELOG.md](CHANGELOG.md)。
