...
├── tools/                    # 11个脚本（含统一菜单menu.py、重点文献卡片literature_cards.py、网页预览器）+ stats/ 统计实现包（9个模块）
...
**当前版本：v1.58**（2026-09-17）
- **定位与陪伴**：AI 从"毕业论文导师"改为**好用的 AI 工具/学习伙伴**——不自称老师、不扮演虚拟伴侣；新增 `core/companionship.md`，参考 AI 情感依赖/"数字伙伴"研究，给稳定但不越界的陪伴，识别情感依赖信号并温和拉回（含未成年人条款）
- **鼓励自然化**：鼓励只给原则、不喂台词，结合学生状态与任务难度自由发挥；去掉夸张人设与机械计数，默认"自然、不套人设"，另留简洁直接/温和耐心/活泼热情三种语气
- **多词检索 + 90 篇候选池**：文献与量表检索必须用多组同义/近义词、中英文/缩写/作者名、概念内 OR/概念间 AND、跨多库综合判断，禁止单词下结论；英文检索双源翻页去重，候选池目标 ≥90 篇（题录层面，≠引用数）
- **重点文献卡片**：新增 `literature_cards.py`（菜单第 10 项），自动去重选重点、生成手机友好的单文件卡片网页
- **手机原生适配**：轻量 Skill 不再是电脑版套壳，按手机真实能力重排，新增设备交接单；Skill 同步到 v1.3

**上一个版本：v1.57** 文献检索闭环：删除与宪法第七条冲突的"AI 读取凭据代填账号密码"档（登录改学生自填/暂不登录两档）、文献链路接进本体、修文献整理器 GBK 静默失败、修打包与测试自身缺陷。

**更早版本（v1.56.5 及以前）的逐版说明全部见
