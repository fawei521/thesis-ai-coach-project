...
> 维护者/接手者：改动后运行 `python tests/full_e2e.py`（约3-5分钟，636 项断言，自动备份恢复测试数据），退出码 0 才算通过；学生日常使用不需要跑。

## 版本

**当前版本：v1.78**（2026-09-19）手机合并单文件构建链路闭环（补丁，学生拿到的内容不变）
- `build_mobile_single.py` 默认输出改到技能目录内，与 `validate.py` 受检的那一份统一；
  从仓库根跑不再把 180 KB 产物落在仓库根（那条路径不在 .gitignore 里）
- 自检报"不同步"时直接给出可执行的重新生成命令；`旧语气名` 断言不再把构建产物当源码问题报；
  `consistency_check.py` 修两处误报（技能脚本的真实开关、构建产物的文档引用）
- **测试**：+2 条行为锁定（真跑子进程钉住"生成方与校验方同一路径"）；full_e2e 632→636 项

**上一个版本：v1.77** 开题产出物闭环：`tools/outline_to_ppt.py` 把大纲排成真 .pptx（只排版不代写）
+ 12 页开题大纲模板、`tools/setup_workspace.py` 把工作区 4→9 目录覆盖全流程；full_e2e 611→632 项；逐条见 CHANGELOG。
