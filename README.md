...
你只需要跟AI说话。需要装软件、跑脚本、检索知网时，AI会一步步带你，或开虚拟电脑帮你做。不想记命令就双击「启动工具箱.bat」。换对话时把 `我的工作区/我的论文进度.md` 发给AI即可无缝续接（这份卡第一次用才生成，包里只带空白模板，所以装新版盖不到你填的东西）。你自己的文献、数据、结果统一放进 `我的工作区/`（已建好九个分类子目录）。拿到新版怎么装：看 `QUICKSTART.md` 第八节。

> **两种使用形态**：① **完整版**（本文件夹，推荐）——含自动化脚本与知识库，按上面 3 步使用；
> ② **豆包 Skill 轻量版**（`doubao-skill/` 目录，Skill v1.10）——可安装到豆包技能目录，只保留分阶段引导协议、身份陪伴边界、鼓励系统与参考资料，**不含脚本**，并对手机做了原生适配，适合不想下载整个项目包、或主要在手机上推进的同学；需要自动化时轻量版会指引回到完整版。两版学术口径一致。
...
├── DEVELOPMENT.md            # 维护者强制开发流程（九阶段门+测试金字塔，改功能前必读）——**只在仓库里，发布包不带**
...
> 维护者/接手者：改中途跑 `python tests/smoke_check.py`（秒级，结构闸）；**哪些改动必须再跑一次
> `python tests/full_e2e.py`**（全量事实与口径断言）见 `DEVELOPMENT.md` 阶段 T 的"跑哪一道闸"表——
> 那张表只有一份，这里不重抄，也**别每改一行就跑全量**。结论只看命令自己打印的末行"失败 0"，
> 项数与耗时都别在文档里抄；学生日常使用不需要跑这些。

## 版本

**当前版本：v1.100**（2026-09-22）**材料区自己守卫生：目录体检挂上菜单第 30 项，分类交给 AI 判断**：`tools/folder_audit.py` 只读扫一遍 `我的工作区/`，八条形制各给位置和「该补什么」——同一个进度卡有两份、机器产物和 .md 摊在同一层、搬完留下的空目录、文件名写着「本次/最终」这类时效词、同层几份却没指出当前版、放了东西却没有 目录.md、顶层冒出第十一个目录、一层出现第二个入口；`--copies <父目录>` 专查这台机器上有几份进度卡在同时被用（两份并存正是乱的根因）。**规则口径改了**：不预先定死类目名字，只锁三件与名字无关的形制——一层只准一个入口（定死叫 目录.md）、一份内容全树只准存在一处（**移动不是删除**）、一个事实只写一处；子层怎么分由你和 AI 判断并写明为什么。依据是同夹具对照实验：自由分类版 6/9 分、四类固定槽位版 4–5/9 分、完全无规则 3/9 分（什么都不做是 4/9）——**格子越硬，越容易削内容去 fit 格子**。每层说明照 `templates/目录模板.md` 四节写，第四节就是可逆清单。细则在 `core/literature-kb.md` 第九、十节。轻量版与手机版不带这条（手机版那个 AI 没有读写本地文件的能力）。

**上一个版本：v1.99**（2026-09-22）**把你读过的文献变成下次还能用的库**：新增第 28 项 `kb_index.py`（按「我读到哪一层」分档、点名缺逐字原文或缺位置的卡、并与第 26 项原文台账对账）与第 29 项 `kb_search.py`（BM25 词法检索＋中文二元切分，**命中给文件名＋行号**，纯标准库、离线、只读）。核心判据：**卡片是定位器、不是独立第二源**，逐字原文只能由学生本人粘贴。逐字见 CHANGELOG。

**上一个版本：v1.98**

**上一个版本：v1.97**（2026-09-21）**"改字"这个动作必须是你自己的**：上一版给了 AI 腔体检和写作留痕，这一版把配套那条判据补成机器查得出来的尺——你图省事说"你直接替我改自然一点，我不看了"，AI 接下这个活**就是不合格**：它改完交上去的还是 AI 的话，风险全在你身上。合格的做法是把体检点出的行号列给你、你自己逐句改，卡住的那句它带你顺，但话得你说出口。另一处变化在维护侧（版本史归档的引用不再靠人手工核对），学生侧零操作变化，工具与菜单都不需要重新学。

**上一个版本：v1.96**（2026-09-21）**"去 AI 味"给的是工具，不是降率**：新增菜单第 24 项 AI 腔体检（读草稿，指出句首套话、每段一样长、三连排比、通篇没有你自己的数据——只报问题位置，不替你改一个字）和第 25 项写作留痕（每版草稿记一行时间/字数/内容指纹，攒"这论文是我一步步写的"过程证据，草稿一个字节都不动）。改文字按 `workflows/writing-guide.md` 第三节的四步走，判据是"有没有只有作者写得出的具体信息"。**本包不测 AIGC 率、不做降率或骗过检测**，也不给"把句子洗成不像 AI"的套路——那是拿你的学位去赌；真被检测误伤，救你的是过程证据与如实申报。轻量版同步出 Skill v1.10。
...
> 别再在这里抄第二遍——两处各写一份，读哪条照做都不算错，那就等于没规定。
