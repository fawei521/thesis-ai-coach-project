...
├── tools/                    # 15个脚本（含统一菜单menu.py、重点文献卡片literature_cards.py、网页预览器、去标识化anonymize_data.py、效应量换算effect_size.py、聚合区分效度validity_cr_ave.py、预试项目分析item_analysis.py）+ stats/ 统计实现包（9个模块）
...
**当前版本：v1.59**（2026-09-18）
- **数据隐私闸**：新增 `anonymize_data.py`（菜单第 11 项），外发前假名化/删除姓名、学号、手机等直接标识符，并对性别/年级/专业/生源组合做 k-匿名体检；只读原文件、另存新文件、绝不覆盖原始数据
- **多元异常值严谨处理**：`auto_stats.py` 新增 `--mahalanobis`，按 Mahalanobis D²/χ² 临界值**只标记不删除**，附含/不含异常值的敏感性分析建议，不为"好看"删数据
- **效应量必报可复核**：新增 `effect_size.py`（菜单第 12 项），由两组均值标准差或 t/F/χ²/r 补算 Cohen's d/Hedges' g/配对 d_z、r 的 Fisher 置信区间、η²/ε²、Cramér's V/φ，支持 r↔d，阈值口径与 stats-guide 一致；只做换算、数字须来自真实输出
- **量表库扩到 29 组（10 大类）**：v1.59 新增 DASS-21、压力知觉、应对方式、情绪调节、自我控制、核心自我评价、生活满意度、基本心理需要 8 节并补 BSMAS；v1.60 再补 PANAS 正负情绪、IRI-C 共情、GQ-6 感恩、GHQ-12 一般健康、PSQI 睡眠质量（新开"睡眠与心身健康类"）；题数/计分/分级硬事实逐条联网核查，口径有出入处标注"以所引原文为准"
- **第 4 个网页范例**：问答式「统计方法选择器」，按研究问题/变量类型/组数/正态方差齐/配对与否向导推荐方法、前提、应报告统计量与效应量阈值、脚本与 JASP/SPSS 路径及留空论文句式（单文件零外链，带"范例请勿直接使用"标注）

**上一个版本：v1.58** 定位为 AI 工具/学习伙伴（去老师化、稳定陪伴不越界、鼓励自然化）＋多词检索与 ≥90 篇候选池＋重点文献卡片网页（`literature_cards.py`，菜单第 10 项）＋手机原生适配（Skill v1.3）。

**更早版本（v1.57 及以前）的逐版说明全部见
