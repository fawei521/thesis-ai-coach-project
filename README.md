...
**当前版本：v1.99**（2026-09-22）**把你读过的文献变成下次还能用的库**：新增第 28 项 `kb_index.py`（清点 `我的工作区/10-知识库/`：按「我读到哪一层」分档、点名哪张卡缺逐字原文或缺位置、并与第 26 项原文台账对账）与第 29 项 `kb_search.py`（词法检索 BM25＋中文二元切分，**命中给文件名＋行号**，纯标准库、离线、零安装、只读不写盘）。卡片模板一篇一个文件，四格对齐引用四要素＋一格「学生核对过没有」。规则单列成 `core/literature-kb.md`（按需读，不进每轮必读），核心判据是**卡片是定位器、不是独立第二源**：摘要级的卡不因「在我库里」而升格，逐字原文只能由学生本人粘贴。两条路线共用同一批文件——甲档不装任何软件，乙档用 Obsidian 打开同一个文件夹（升级不换格式、不搬家）；向量检索按实测代价评估后默认不做并写明理由。上一版 v1.98 解决「这篇有没有全文」，本版解决「我核过的那一句在哪」。轻量版出 Skill v1.10。
