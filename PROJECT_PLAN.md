...
- **v1.38** ✅ 菜单产物归位、正态性引导与版权合规（当前）：菜单4英文检索默认归位工作区01、paper_search自动建目录（回归13断言、联网实测）；stats-guide明确偏度峰度Kline判据边界、正式正态检验引导JASP/SPSS；移除Sci-Hub侵权引导改合法获取途径；走查确认统计文档与代码能力一致、中英文检索路径区分清楚
