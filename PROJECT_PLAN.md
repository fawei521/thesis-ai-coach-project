...
- **v1.36** ✅ 缺库优雅降级固化＋菜单1端到端回归（当前）：走查确认无 matplotlib/numpy/scipy 时 auto_stats 纯标准库数值照常产出仅跳过图表并提示、chart 友好退出1；新增 tests/test_graceful_degradation.py（9断言）永久回归；菜单1问卷星预处理端到端验证
