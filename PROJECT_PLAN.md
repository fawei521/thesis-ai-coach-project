...
- **v1.27** ✅ 相关矩阵热图（当前）：auto_stats跑量表总分相关时自动导出_相关热图.png（下三角系数+显著性星号、对角三位Cronbach α、上三角留白、红蓝发散色标，300dpi可插图），--spearman同步为秩相关热图；系数与已黄金验证的相关矩阵逐元素一致（numpy差<1e-15），matplotlib/numpy软依赖缺失静默跳过
