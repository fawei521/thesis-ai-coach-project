...
- **v1.8** ✅ 预置学生工作区（commit 3cc450d）：
  - 新增 `我的工作区/`（01-文献PDF/02-问卷数据/03-分析结果 + 先读我），进度卡直接就位，解压即有清晰归档
...
- **v1.24** ✅ 数据质量增强：data_cleaner新增注意力检查题（IRI，答错即剔除）、长直线longstring（阈值可配）、个体内低变异SD、缺失率过高；质量指标只对量表/Likert作答题（--scales最准，否则启发式排除时长/人口学列）；新增剔除明细报告_清洗报告.csv并按原因汇总；合成数据逐行黄金
- **v1.25** ✅ 文档↔代码一致性自检：新增tests/consistency_check.py，AST解析各工具真实CLI开关与导出csv，交叉核对全部md的脚本引用/开关/导出文件名三类漂移；纳入测试金字塔L7与发布清单（退出码0才打包）；变异测试验证能抓假脚本/假开关/假导出且零误报
- **v1.26** ✅ 文档导航完整性：consistency_check增加第4类——markdown链接与反引号引用的.md必须真实存在（裸文件名全项目同名兜底）、命令行脚本（含tests/裸调用）核对存在性、词边界排除URL与通配符误切；全项目106处引用零悬空，变异可抓假文档链接
- **v1.27** ✅ 相关矩阵热图：auto_stats跑量表总分相关时自动导出_相关热图.png（下三角系数+显著性星号、对角三位Cronbach α、上三角留白、红蓝发散色标，300dpi可插图），--spearman同步为秩相关热图；系数与已黄金验证的相关矩阵逐元素一致（numpy差<1e-15），matplotlib/numpy软依赖缺失静默跳过
- **v1.28** ✅ 真人视角走查·跨阶段衔接修复：小白视角从START走查全程；问卷模板补注意力检查题+作答时长埋点（清洗输入在发问卷前布置）；coach-rules阶段5埋点/阶段6样本量/阶段7五指标/阶段8能力同步；QUICKSTART菜单补第8项、流程图顺序按12阶段纠正；全项目工作区目录名统一为带连字符真实名、知网/PDF落点修正；START版本号与步骤编号修正；consistency_check增第5类工作区路径检查、新增测试25走查断言
- **v1.29** ✅ 后半程真人走查·答辩与写作衔接：答辩题库20→24问（清洗剔除标准、未成年人监护人知情同意/敏感话题保护/数据隐私伦理三问、样本量与问卷质量标准答法）；写作结果章节顺序对齐工具完整输出（正态性/差异/卡方/调节简单斜率/相关热图插图）；走查确认ethics与ai-literacy完整
- **v1.30** ✅ 问卷星多选/填空/哑变量列健壮性：预处理识别多选题（┋分隔/表头标志）与开放填空题并单列"不进量表"，与需人工编码单选分开；清洗器无scales兜底排除纯0/1多选哑变量/是否题（NSSI等0/1量表走scales精确通道），杜绝污染长直线/低变异；新增test_special_columns.py黄金回归（11断言），三处文档补多选/填空说明
- **v1.31** ✅ 紧急模式可执行化＋量表库循证纠错：紧急模式补三条不可破红线与7/3/1天逐日最小任务、先定位卡点；量表库NSSI删除杜撰/张冠李戴的ASFQ、SBI，循证改为FASM(C-FASM)/DSHI(DSHI-9)/ISAS并提示0/1或计数计分，修正UCLA简表出处
- **v1.32** ✅ 论文大纲模板与统计指南走查对齐：大纲结果章补正态性/EFA平行分析/人口学差异(t/Welch/ANOVA/非参数)/卡方/调节简单斜率，研究程序补未成年人监护人知情同意与质量埋点、数据处理补分半与Bootstrap；逐行核实stats-guide与代码一致无夸大；复核AI使用声明合规
- **v1.33** ✅ 开题报告与答辩PPT模板走查对齐：开题样本量改功效分析依据、信度引已有研究不预填α、程序补未成年人伦理/质量埋点/五指标、数据处理补分半/EFA/差异/卡方/非参数/调节、创新点去"首次"夸大；答辩PPT研究对象页补伦理合规、研究空白谨慎措辞、流程补差异
- **v1.34** ✅ 进度卡模板对齐与工具链走查：修复 progress-template 与工作区进度文件断档（阶段7剔除五指标、剔除日志、功效最小N、分半/差异/卡方/调节结果栏）；走查 Zotero+Obsidian 工具链、校正 Better BibTeX 依赖表述
- **v1.35** ✅ 工具箱菜单4/5/6/7真人端到端走查与模型图崩溃修复：修复 chart_generator 的 --coefs 必填导致菜单第6项回车即报错退出，改为可省略自动补0、数量不符补齐/截断、非数字友好报错、去 SimHei bold 警告；实测菜单4英文检索(OpenAlex联网)/5文献整理/7演示数据均可用
- **v1.36** ✅ 缺库优雅降级固化＋菜单1端到端回归：走查确认无 matplotlib/numpy/scipy 时 auto_stats 纯标准库数值照常产出仅跳过图表并提示、chart 友好退出1；新增 tests/test_graceful_degradation.py（9断言）永久回归；菜单1问卷星预处理端到端验证
- **v1.37** ✅ 演示数据练手闭环＋启动器路径健壮性：走查菜单7全新生成→菜单3统计闭环自洽（链式显著、CSV与热图齐全）并固化为回归（扩到12断言）；菜单7回车默认归位工作区02不污染源码根；确认启动器与菜单从任意目录启动均能定位脚本
- **v1.38** ✅ 菜单产物归位、正态性引导与版权合规：菜单4英文检索默认归位工作区01、paper_search自动建目录（回归13断言、联网实测）；stats-guide明确偏度峰度Kline判据边界、正式正态检验引导JASP/SPSS；移除Sci-Hub侵权引导改合法获取途径；走查确认统计文档与代码能力一致、中英文检索路径区分清楚
- **v1.39** ✅ AI素养教育增强＋能力清单真实性走查：ai-literacy 补"会用工具的AI（联网/虚拟电脑/跑脚本）"能力边界去魅与关键动作人工确认、隐私去标识化；走查START能力清单VIF/Bonferroni/Welch/分半/EFA/简单斜率均真实实现无夸大
- **v1.40** ✅ 心理援助热线事实纠错与危机处置：联网核实全国统一心理援助热线12356（国卫医政函〔2024〕259号，2025年5月一号接通）列为首选；修正400-161-9995误标官方全国热线（实为公益希望24热线）；ethics/问卷模板/答辩指南统一补即刻危险拨120/110急诊
...
- **v1.49** ✅ Zotero/Obsidian 工具链纠错：ZI插件只导入文献笔记，正文引用改指 Word Zotero插件（推荐）/Citations/Zotero Citations；修full_e2e清理遗漏，断言增至118
- **v1.50** ✅ 数据库批量下载红线＋归类/结果顺序：联网核实知网过量下载阈值与封禁规则，补合规红线（单次≤约30篇、全天≤30-50、禁爬虫/整刊、全文不传播）；题录总表统一归01；写作结果章人口学差异独立；断言122
- **v1.51** ✅ 未成年人研究伦理补强：assent按年龄分层且与监护人许可分开、尊重dissent，新增中学生书面assent模板、研究前风险转介预案、校园投放伦理；数据保存改为毕业后≥5年/科研5-7年；断言125
- **v1.52** ✅ 统计指南纠错与增强：stats-guide 的 PROCESS 官网订正为 processmacro.org（v1.46漏改）；网络分析补 strength/expected influence 与 bootnet 边权/删案例自助 CS 系数稳定性、定位补充分析；断言127
- **v1.53** ✅ 问卷模板人口学适配混合样本（当前）：性别加其他/不愿透露、新增学段题、年级中学版/大学版二选一、专业题标注仅大学生、个人信息最小化；断言129
