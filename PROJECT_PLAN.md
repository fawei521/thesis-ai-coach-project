...
- **v1.30** ✅ 问卷星多选/填空/哑变量列健壮性（当前）：预处理识别多选题（┋分隔/表头标志）与开放填空题并单列"不进量表"，与需人工编码单选分开；清洗器无scales兜底排除纯0/1多选哑变量/是否题（NSSI等0/1量表走scales精确通道），杜绝污染长直线/低变异；新增test_special_columns.py黄金回归（11断言），三处文档补多选/填空说明
