...
- **v1.28** ✅ 真人视角走查·跨阶段衔接修复（当前）：小白视角从START走查全程；问卷模板补注意力检查题+作答时长埋点（清洗输入在发问卷前布置）；coach-rules阶段5埋点/阶段6样本量/阶段7五指标/阶段8能力同步；QUICKSTART菜单补第8项、流程图顺序按12阶段纠正；全项目工作区目录名统一为带连字符真实名、知网/PDF落点修正；START版本号与步骤编号修正；consistency_check增第5类工作区路径检查、新增测试25走查断言
