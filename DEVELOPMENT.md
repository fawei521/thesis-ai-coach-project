...
> **L5 一键回归用法**：`python tests/full_e2e.py` 把统计/清洗/样本量/模型图/文献脚本的真实运行、统计基准数值、缺库降级闭环、文档-代码一致性、合规与量表事实断言、doubao-skill 轻量版自检（含植入变异的阴性测试）全部跑一遍（项数只增不减；**当前项数与耗时以它自己打印的末行为准，别在文档里抄数字**——本句曾经写着"218 项起""约3-5分钟"，两个都漂了），结论看末行"失败 0"。脚本自动定位项目根、自动备份并恢复 `tests/test-data` 基准样例、自动清理生成物，因此开发仓库与解压后的干净副本都能直接跑；新增能力必须同步加断言、不允许只加功能不加回归；**断言写在 `tests/e2e_cases/` 下的 `case_NN` 对应主题片段里**（v1.82 起：612+ 条断言按版本主题切成顺序片段，每片 ≤220 行正文，新增主题就加一个编号并挂进 `FRAGMENTS`，`tests/full_e2e.py` 只剩骨架 + `FRAGMENTS` 顺序清单的壳），片段由壳按序 `exec` 进同一命名空间，`check/run/tx/ROOT/TD` 与前面各段产出的变量都在里面。
