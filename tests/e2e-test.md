...
2. **六项硬口径两侧同源**：`python tests/skill_sync_check.py` 退出码 0（简单中介 ≥200／链式 ≥300／Harman 40%／Bootstrap 5000／12356／五指标）；
...
4. **尺子本身要能被检测空转**：`python tests/skill_sync_check.py --selftest` 退出码 0——七项植入变异全部被抓、正常输入不误报。
