...
| 测试97 | 原文可得性探测与题录双源核验（v1.98，case_24.py） | 本文件 |
...
## 测试97：原文可得性探测与题录双源核验（v1.98，`case_24.py` 全离线）

**做法**：把两个新工具的取数函数换成假响应（PDF 字节流 / 开放全文 XML / HTML 壳 / 抛异常），
只验判定逻辑——**回归不联网**，真实网络只在学生机器上跑一次并留台账。

1. 两个工具纯标准库、编码守卫按房内形制（含 `输出编码守卫` 标记与 stderr）、无凭据处理代码。
2. 菜单第 26/27 项接线、处理器落在 `menu_ref.py` 且被 `menu.py` 点名、`START.md` 登记。
3. 两份模板带全列头，且**自带阴性样本**（一条故意错的 DOI、两条编造标题）——模板没阴性样本＝尺可能空转。
4. 六态：可获取／可获取(全文XML)／需数据库／非PDF／失败／**标题对不上**；
   其中"标题对不上"排在下载之前（DOI 指向另一篇时**不许**去下载）。
5. 五态：双源一致／有出入（标题、刊名分别点名）／双源互斥／单源／登记处查无；
   单源与查无都**不**升级为"已核实"。
6. 缺字段只留空位不猜值；`# 说明行`不算数据（CSV 两种编码都读得进）。
7. 第 23 项把 `【你来写】`类模板占位与 `【缺：卷号】`类核验标注**分开数**，
   `[未查到]/[推断]` 只给提示不判缺项——**空着是事实，填上才是编**。

**这一版踩到的两处**（留给下一个人，别再来一遍）：
- 我给 Brand 2019 手填 DOI 时把尾号猜成 …06.023，Crossref 返回一篇讲孤独症退化的文章，
  格式完整、内容全错。**所以两把尺都先做"标题回核"**；`same_title` 第一版拿前 36 字符硬比，
  把"I-PACE model…"与"The I-PACE model…"判成不同篇——改成互相包含＋相似度 ≥0.8。
- 片段共享壳的 globals：`case_18.py:70` 把 `st` 顶成了轻量版 self-test，
  我在末段片段里用 `st` 断言 START.md 登记，差点变成"永远通过"的空转——**跨片段变量名要当全局看**。