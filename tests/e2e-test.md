...
## 测试24：文档↔代码一致性自检（v1.25 起，v1.26 增文档导航）
...
**变异测试（验证检查器不空转）**：临时新建一个 md，写入一个不存在的脚本路径（形如 tools/ 后跟一个编造的文件名）、一个未定义的长开关（形如两个连字符后跟编造的英文开关名）、一个不存在的中文导出 csv 名、一个指向编造文档名的链接，脚本必须退出码 1 且逐条报出这些漂移；删除临时文件后恢复 0。注意变异用的名字要用编造占位、不要与真实文件重名，避免与正常文档混淆。
