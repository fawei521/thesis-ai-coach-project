...
| 测试90 | py_compile 并入回归、待办编号在本仓可解析（未发版，case_02.py / case_03.py / 维护档案/待办编号索引.md） | 本文件 |
| 测试91 | 门禁按改动面分层：秒级 smoke 与全量各跑什么时候（未发版，smoke_check.py / case_02.py） | 本文件 |
| 测试92 | 门禁凭证审计：动了 L1 却没跑全量，事后会被判红（未发版，case_21.py / smoke_check.py / DEVELOPMENT.md 阶段 T 表） | 本文件 |
...
量表上、搜漏一次就宣布"原文没报"。这类要求旧版散在多页且全是软措辞，v1.90 收进 `core/evidence-rigor.md` 并接进每轮门禁。
...
## 测试90：门禁收口成一条命令、待办编号在本仓可解析（外部建议评估后的止血，未发版）
...
1. `py_compile全通过` — 用回归现成的 `run()` 编译 `tools/`、`tests/`、`doubao-skill/` 下全部 `.py`。
   ...
   这是此前唯一没被回归覆盖的一道；补上后 `python tests/full_e2e.py` 一条即全部。
...
**同批改到、也属于本用例的守卫翻转**：`case_03.py` 的 `README数字修正` 原来断言 `"16种" in README`——
那是当年为修"10种统计方法"钉下的"正确答案"，而真值后来 29→39 变了两次，README 那个 16 因为"有断言保护"
一直没人动。**断言钉住某个时刻的具体值＝给漂移续命**。现改为反向钉：`16种`/`24组`/`10种统计方法` 一律不得出现。
...
**后续订正（同日晚，见测试91）**：本条 ③ 原本把结论写成"门禁收口成**一条命令**＝只跑 `full_e2e`"。
实测全量只要 69.7 秒（文档写的"约 3-5 分钟"漂了 3 倍），但**"一条命令"这个 framing 本身是错的**——
它等于鼓励每完成一小步就跑全量。现按改动面分层（见测试91）。本条 ① 的 py_compile 并入回归、
② 的待办编号可解析仍然有效，四条断言一个字没动。

## 测试91：门禁按改动面分层——秒级 smoke 与全量各在什么时候跑（未发版）
