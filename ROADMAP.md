...
- [x] 自行完成整个系统测试，走一遍流程生成一篇模拟论文，收集流程中遇见的 bug 修改，循环至少三次——**v1.64 已完成三轮**（标准路径 29 步、边界坏参 34 步、修复后干净重跑＋产物链核对），过程与结论见 `维护档案/CHANGELOG-历史详情-v1.0-v1.75.md` 的 v1.64 条、`tests/e2e-test.md` 测试63

- [ ] **把 v1.100 的 A/B 实验搬进 `tests/`**（v1.100 交付时只带上了规则与尺子）：实验脚本现在还在开发侧
      临时区，243 行超 220 闸门、且缺"输出编码守卫"那行标记；搬进去要压行数，并把行为用例登记成
      T68/T69（接现有 T67，编号必须连续）。**在它进仓之前，这条规则按 `CONSTITUTION.md` 第十条算
      「未完全验证」**——结论（自由分类优于固定槽位、收走原句须留指针）是真的、可复跑，但别人拿到
      仓库重跑不了。
- [ ] 材料区尺子的两处已知盲区：`--copies` 只按同名找权威卡，**改了名字存成两处的情况它覆盖不到**；
      跨副本双卡要人主动指定父目录，扫整个磁盘会掉进代码仓产生假红

## v2.0 计划（中期）
- [ ] 增加实验研究、质性研究方法支持
- [x] **本地文献知识库，攒起来后基于原文回答** → **v1.99 已交付**：卡片格式（逐字原文＋位置＋读到哪一层＋学生核对）＋第 28 项清点对账＋第 29 项词法检索（BM25＋中文二元切分，命中带行号）＋`core/literature-kb.md` 用库规矩；两条路线共用同一批文件（不装软件／Obsidian 打开同一文件夹）。**向量检索按实测代价评估后默认不做**：它必须有 embedding 模型（云 API 或本地 torch 依赖，破坏零依赖承诺），BEIR 实测“BM25 是稳健基线、稠密检索常常不及其它方法”，且向量结果给不出行号、无法逐行核对。**遗留**：卡片上千张后的检索性能、中文库（知网）PDF 抽文字、近义召回的词表补全。
- [ ] 文献自动摘要和笔记生成
- [ ] 多学科扩展（教育学、社会学等社科）
- [ ] 多轮对话状态追踪、项目专属配置文件
