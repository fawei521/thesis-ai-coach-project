...
> **这个模板少三格，而这正是"笔记记了却不敢引"的原因**（v1.99 补）：
> ① **我读到哪一层**（全文PDF／仅摘要／检索结果页／二手转述）——只有摘要时，上面的"关键发现"不能写进论文；
> ② **摘录的位置**（第几节／哪张表／第几页）——没有它，三个月后你无法回到原文验证自己抄对没有；
> ③ **这段是谁取出来的、你核对过没有**——AI 生成的笔记与它当时的印象同源，不能拿来"复核"它自己。
> 补齐这三格的现成模板是 `templates/文献卡片模板.md`（一张卡一篇，放 `我的工作区/10-知识库/`，
> Obsidian 可以直接把这个文件夹当仓库打开）。**两份模板并存时以它为准**，理由见 `core/literature-kb.md` 第二节。
