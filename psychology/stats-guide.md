...
**前提不满足**：差值明显偏态或样本很小时用 **Wilcoxon 符号秩检验**（配对非参数）：n≤25 且无结用精确 p，否则用正态近似 z（含结校正与连续性校正，与 SPSS 口径一致），报告 `W+、z、p`；它检验的是差值分布是否关于 0 对称（位置移动），不直接检验均值差。
...
- 报告格式：t(n−1)=X.XX, p=.XX, d_z=.XX（95%CI[.,.]）；Wilcoxon：W+=XX, z=X.XX, p=.XX
