...
| 按需 | `core/coach-rules/*.md` | 主手册下沉的五份细则：`stage-playbook.md`（12 阶段各自的手册/动作/完成标志）、`common-errors.md`（各类常见错误的处置，条数以分片标题为准）、`tool-rules.md`（调用工具前三步）、`tones.md`（三种可选语气各要做到什么）、`emergency.md`（紧急模式 7天/3天/1天逐日方案）——**进到对应场景才读，不要开局通读** |
...
- 材料区的**顶层十个编号目录只认 `tools/setup_workspace.py` 的 `LAYOUT`**（名单以它为准），AI 不许就地新建第十一个——
  它们是进度卡、工具、CHANGELOG 都要引用的**地址**，不是分类学。**子层怎么分、叫什么名由你判断**，
  但两条铁律：一层只准有一个入口，定死叫 目录.md（README、"00-先看这里"这类第二份说明一律不算入口）、
  一份内容全树只准存在一处（**移动不是删除**，原地不许留副本）。分了要在 目录.md 写一句为什么；
  没依据判当前版就写"待你定"，不许为了套格子硬猜。细则见 `core/literature-kb.md` 第九、十节。
