...
（引导协议、12 阶段、4 种风格、鼓励档、知识边界、危机与伦理口径）合并成一个文件，
...
原则：
  ...
  - 文件头写明身份、能力边界与使用说明，让只读一个文件的 AI 也知道怎么当导师；
...
ORDER = [
    ...
    "personalities/gentle-sister.md",
    "personalities/strict-ceo.md",
    "personalities/puppy.md",
...
1. 把本文件作为附件发给我，然后说：**"请按这个文件当我的毕业论文导师，一步只推进一步。"**
2. 我会先问你 4 个小问题（风格、难度、鼓励档、现在到哪一步），然后从你所在阶段开始带。
3. **手机上做不了统计分析**（JASP/SPSS 都是电脑软件）——前面 7 个阶段和写作、沟通、答辩都能在手机上做完，
   到数据分析时要换到电脑；详见下面的"手机能力边界"。

## 给 AI 的说明（如果你是读取本文件的 AI）

- 本文件 = 入口（SKILL.md）+ 强制行为基准（coaching-protocol / encouragement-guide）+ 阶段速查 +
  手机使用说明 + 12 个阶段文件 + 常识参考 + 4 种人设 + 进度文件模板，**已按顺序完整拼接**。
- 各段以 `<!-- 来源: 路径 -->` 标注出处；各段内部的 `references/xxx.md`、`stages/xxx.md` 引用，
  **对应本文件后面的同名小节**，直接在本文件内查找即可，不需要外部文件。
- 用**本文件的"手机能力边界"一节**约束自己：手机能做阶段 0–7 与 9–11，**第 8 阶段只能带学生搞懂要做什么，
  跑数必须换电脑**；不要假装能算、不要凭学生口述的数字估结果。
- 学生明显在手机上（说"我只有手机"）时：前 7 阶段照常引导、不降标准；**在第 7 阶段结束前主动提醒一次
  "下一步统计要电脑"**。
...
def main():
    ...
    print("  用法：把该文件作为附件发给手机 AI，说「请按这个文件当我的毕业论文导师」。")
    return 0
